...
function stringFormat(template$$1, binding) {
    const /** @type {?} */ compiled = _.template(template$$1);
...
class McBreadcrumbsService {
    ...
    constructor(_router, route, _config, _injector) {
        ...
        this._router.events
            ...
            this._resolveCrumbs(currentRoot).pipe(mergeMap((x) => x), distinct((x) => x.text), toArray(), mergeMap((x) => {
    ...
    _resolveCrumbs(route) {
        ...
        if (data && data.breadcrumbs) {
            ...
            const /** @type {?} */ result = resolver.resolve(route, this._router.routerState.snapshot);
